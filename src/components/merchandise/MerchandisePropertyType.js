...
const columns = [
  { id: 'merch_property_type_id', label: 'Property Type ID', minWidth: 50 },
  { id: 'merch_type_id', label: 'Type ID', minWidth: 100 },
  { id: 'merch_property_type_name', label: 'Property Name', minWidth: 20 },
  { id: 'merch_type_name', label: 'CategoryName', minWidth: 20 },
...
const MerchandisePropertyType = () => {
  ...
  useEffect(() => {
    const getProperty = async () => {
      const res =
        await axios.get(`http://localhost:8082/merchandisePropertyType/queryAllMerchandisePropertyType`, {
          params: {
            page: page,
  ...
  const addNewFunction = (e) => {
    ...
    axios.post(`http://localhost:8082/merchandisePropertyType/addNewMerchandisePropertyType`, qs.stringify({
        merchPropertyTypeName: addTypeName,
  ...
  const deleteFunction = (Id) => {
    axios.delete(`http://localhost:8082/merchandisePropertyType/removeMerchandisePropertyType`, {
      params: {
        merchandisePropertyTypeId: Id,
      }
    }).then(function (response) {
      if (response.data) {
  ...
  const getEditFunction = (Id) => {
    axios.get(`http://localhost:8082/merchandisePropertyType/getMerchandisePropertyTypeById`, {
      params: {
        merchandisePropertyTypeId: Id,
      }
    }
    ).then(function (response) {
      setEditID(response.data.t.merch_property_type_id);
      setEditName(response.data.t.merch_property_type_name);
  ...
  const editFunction = (e) => {
    ...
    axios.post(`http://localhost:8082/merchandisePropertyType/editMerchandisePropertyTypeById`, qs.stringify({
        merchPropertyTypeId: editID,
        merchTypeId: categorySelect['id'],
        merchPropertyTypeName: editName,
    })).then(function (response) {
      if (response.data) {
        setSuccessEdit(true);
  ...
  const searchFunction = () => {
    ...
    {
        axios.get(`http://localhost:8082/merchandisePropertyType/searchMerchandisePropertyType`, {
            params: {
                    page: page,
                    rowsPerPage: rowsPerPage,
        ...
        <Fade in={addOpen}>
          <Box sx={style}>
            <form onSubmit={addNewFunction}>
              ...
              />
              <TextField required fullWidth label="Property Type Name" value={addTypeName}
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {rows
                .map((row) => {
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={row.merch_property_type_id}>
                      ...
                      <TableCell key='3' align={columns[3].align}>
                        <Button variant="outlined" color="error" onClick={() => { deleteFunction(row.merch_property_type_id) }}>
                          <DeleteIcon />Delete
                        </Button>
                        <Button variant="outlined" onClick={() => { setEditOpen(true); getEditFunction(row.merch_property_type_id); queryAllCategories() }}
...
export default MerchandisePropertyType
